#!/usr/bin/env python3
"""
CatGPT 2.x — Multi‑Language Programming IDE
════════════════════════════════════════════
600×400 Tkinter IDE with built‑in C compiler (lexer+parser+interpreter)
and external support for many programming languages.

Features:
  - Built‑in C compiler (no external tools needed) with full C syntax
  - External compiler/interpreter support for:
      C, C++, Python, Java, JavaScript, Ruby, Go, Rust, and more
  - Language selection via dropdown; automatic file extension handling
  - Syntax highlighting for C (other languages plain text)
  - Run, Compile, Build+Run for each language using system tools
  - Optional GCC integration for C/C++ (assembly, etc.)
  - Dark theme, line numbers, output panel

Run: python catgpt2x.py
"""
...
# ═══════════════════════════════════════════════════════════════════════
# AST NODES (unchanged, used only for built‑in C)
# ═══════════════════════════════════════════════════════════════════════
    ...
    def parse(self):
        ...
        while not self.match(TK_EOF):
            ...
            if self.match(TK_PUNCT) and self.cur().value.startswith('#'):
                self.eat()
                if not self.match(TK_EOF): self.eat()
                continue
...
# Colors — dark theme (Catppuccin Mocha) – unchanged
BG_DARK    = "#1e1e1e"
BG_EDITOR  = "#1e1e2e"
BG_GUTTER  = "#16161e"
BG_OUTPUT  = "#11111b"
...
LANG_CONFIG = {
    'C': {
        'ext': '.c',
        'compile': ['gcc', '{src}', '-o', '{exe}'],
        'run': ['{exe}'],
        'build_run': ['gcc', '{src}', '-o', '{exe}', '&&', '{exe}'],  # shell‑style
        'assembly': ['gcc', '-S', '{src}', '-o', '{asm}'],
        'single_step': False,
    },
    'C++': {
        'ext': '.cpp',
        'compile': ['g++', '{src}', '-o', '{exe}'],
        'run': ['{exe}'],
        'build_run': ['g++', '{src}', '-o', '{exe}', '&&', '{exe}'],
        'assembly': ['g++', '-S', '{src}', '-o', '{asm}'],
        'single_step': False,
    },
    'Python': {
        'ext': '.py',
    ...
    'Java': {
        ...
        'compile': ['javac', '{src}'],
        'run': ['java', '{classname}'],
        'build_run': ['javac', '{src}', '&&', 'java', '{classname}'],
        'assembly': None,
        'single_step': False,
        'classname': lambda src: os.path.splitext(os.path.basename(src))[0],
    },
    'JavaScript': {
        'ext': '.js',
        'compile': None,
    ...
    'Go': {
        ...
        'compile': None,
        'run': ['go', 'run', '{src}'],
        'build_run': ['go', 'run', '{src}'],
        'assembly': None,
        'single_step': True,
    },
    'Rust': {
        'ext': '.rs',
        'compile': ['rustc', '{src}', '-o', '{exe}'],
        'run': ['{exe}'],
        'build_run': ['rustc', '{src}', '-o', '{exe}', '&&', '{exe}'],
        'assembly': None,
        'single_step': False,
    },
...
class CatGPT_GUI:
    def __init__(self, root):
        ...
        # Language selection
        self.current_lang = tk.StringVar(value="C")
        self.lang_list = list(LANG_CONFIG.keys())

        # GCC extension detection (still used for C/C++)
        self.gcc_available = self._check_gcc()
        ...
        self._update_gutter()
        self._highlight()
        self._status("Ready — Built‑in C / Multi‑language external", "ok")

    # ─── External tool helpers ───────────────────────────────────────
    def _check_gcc(self):
        gcc = shutil.which('gcc')
        gpp = shutil.which('g++')
        return gcc is not None or gpp is not None

    def _check_tool(self, lang, cmd_name):
        """Check if the primary command for a language is available."""
        # cmd_name is the first word of the run/compile command (e.g., 'python', 'javac')
        return shutil.which(cmd_name) is not None

    def _run_external(self, mode='run'):
        """Run external compiler/interpreter for the selected language."""
        lang = self.current_lang.get()
        if lang not in LANG_CONFIG:
            self._out(f"Unsupported language: {lang}\n", "stderr")
            return

        cfg = LANG_CONFIG[lang]
        source = self.editor.get("1.0", "end-1c")
        if not source.strip():
            self._out("No source code.\n", "stderr")
            return
        ...
        # Prepare command list (replace placeholders)
        if mode == 'compile' and cfg.get('compile') is None:
            self._out(f"Language {lang} does not support separate compilation.\n", "stderr")
            return
        if mode == 'assembly' and cfg.get('assembly') is None:
            self._out(f"Assembly generation not supported for {lang}.\n", "stderr")
            return

        if mode == 'assembly':
            cmd_tmpl = cfg['assembly']
        elif mode == 'compile':
            cmd_tmpl = cfg['compile']
        elif mode == 'run':
            cmd_tmpl = cfg['run']
        else:  # build_run
            cmd_tmpl = cfg['build_run']

        if cmd_tmpl is None:
            self._out(f"No command defined for {mode} with {lang}.\n", "stderr")
            return
        ...
        for part in cmd_tmpl:
            if part == '{src}':
                ...
                cmd.append(asm_path)
            elif part == '{classname}' and 'classname' in cfg:
                cmd.append(cfg['classname'](src_path))
            else:
                cmd.append(part)

        # For shell‑style &&, we need to handle separately
        if '&&' in cmd:
            ...
            self._out(f"$ {' '.join(cmd)}\n", "info")
            self._status("Running external...", "info")

            try:
                # Execute each part sequentially
                for i, part_cmd in enumerate(parts):
                    proc = subprocess.run(part_cmd, capture_output=True, text=True, timeout=30)
                    if proc.returncode != 0:
                        self._out(proc.stderr + '\n', "stderr")
                        self._status(f"Step {i+1} failed", "err")
                        return
                    if proc.stdout:
                        self._out(proc.stdout, "stdout")
                self._status("Done", "ok")
            except subprocess.TimeoutExpired:
                self._out("Timeout.\n", "stderr")
                self._status("Timeout", "err")
            except FileNotFoundError as e:
                self._out(f"Tool not found: {e.filename}\n", "stderr")
                self._status("Missing tool", "err")
            except Exception as e:
                self._out(f"Error: {e}\n", "stderr")
                self._status("Error", "err")
            finally:
                self._cleanup_temp(src_path, exe_path, asm_path)
            return
        ...
        try:
            ...
            if proc.stdout:
                self._out(proc.stdout, "stdout")
            if proc.stderr:
                self._out(proc.stderr, "stderr")
            if proc.returncode != 0:
                self._status(f"Failed with code {proc.returncode}", "err")
            else:
                self._status("Done", "ok")
        except subprocess.TimeoutExpired:
            self._out("Timeout.\n", "stderr")
    ...
    def _cleanup_temp(self, src, exe, asm):
        try:
            os.unlink(src)
    ...
    def _build_menu(self):
        ...
        bm = tk.Menu(mb, **mcfg)
        bm.add_command(label="Compile (C built‑in)      F5", command=self.do_compile)
        bm.add_command(label="Run (C built‑in)          F6", command=self.do_run)
        bm.add_command(label="Build & Run (C built‑in)  F9", command=self.do_build_run)
        bm.add_separator()
        bm.add_command(label="Stop", command=self.do_stop)
        bm.add_separator()
        bm.add_command(label="Run External (current lang)   Ctrl+R",
                       command=lambda: self._run_external('run'))
        bm.add_command(label="Compile External (if supported) Ctrl+E",
                       command=lambda: self._run_external('compile'))
        bm.add_command(label="Build & Run External        Ctrl+B",
                       command=lambda: self._run_external('build_run'))
        mb.add_cascade(label="Build", menu=bm)

        gm = tk.Menu(mb, **mcfg)
        gm.add_command(label="Compile with GCC (C)   F7",
                       command=lambda: self._run_external_lang('compile', 'C'),
                       state='normal' if self.gcc_available else 'disabled')
        gm.add_command(label="Run with GCC (C)       F8",
                       command=lambda: self._run_external_lang('run', 'C'),
                       state='normal' if self.gcc_available else 'disabled')
        gm.add_command(label="Build & Run with GCC (C) F10",
                       command=lambda: self._run_external_lang('build_run', 'C'),
                       state='normal' if self.gcc_available else 'disabled')
        gm.add_separator()
        gm.add_command(label="Generate Assembly (C)  F11",
                       command=lambda: self._run_external_lang('assembly', 'C'),
                       state='normal' if self.gcc_available else 'disabled')
        gm.add_separator()
        gm.add_command(label="Compile C++ with GCC",
                       command=lambda: self._run_external_lang('compile', 'C++'),
                       state='normal' if self.gcc_available else 'disabled')
        gm.add_command(label="Run C++ with GCC",
                       command=lambda: self._run_external_lang('run', 'C++'),
                       state='normal' if self.gcc_available else 'disabled')
        mb.add_cascade(label="GCC", menu=gm)

        lm = tk.Menu(mb, **mcfg)
        for lang in self.lang_list:
            lm.add_command(label=lang, command=lambda l=lang: self.current_lang.set(l))
        mb.add_cascade(label="Language", menu=lm)
    ...
    def _run_external_lang(self, mode, lang):
        """Run external command for a specific language (bypass current selection)."""
        old = self.current_lang.get()
        self.current_lang.set(lang)
        self._run_external(mode)
        self.current_lang.set(old)
    ...
    def _build_toolbar(self):
        ...
        tk.Button(tb, text="C-COMP [F5]", command=self.do_compile,
                  bg=BG_BUTTON, fg="#a6e3a1", activebackground=BG_BTN_HVR,
                  activeforeground=FG_TEXT, relief="flat", bd=0, padx=8, pady=2,
                  font=("Consolas", 9, "bold")).pack(side="left", padx=2, pady=3)
        tk.Button(tb, text="C-RUN [F6]", command=self.do_run,
                  bg=BG_BUTTON, fg="#89b4fa", activebackground=BG_BTN_HVR,
                  activeforeground=FG_TEXT, relief="flat", bd=0, padx=8, pady=2,
                  font=("Consolas", 9, "bold")).pack(side="left", padx=2, pady=3)
        tk.Button(tb, text="C-BUILD [F9]", command=self.do_build_run,
        ...
        # Language selector
        ttk.Combobox(tb, textvariable=self.current_lang, values=self.lang_list,
                     state="readonly", width=8, font=("Consolas", 9)).pack(side="left", padx=4)

        tk.Button(tb, text="RUN EXT", command=lambda: self._run_external('run'),
                  bg=BG_BUTTON, fg="#cba6f7", activebackground=BG_BTN_HVR,
                  activeforeground=FG_TEXT, relief="flat", bd=0, padx=8, pady=2,
                  font=("Consolas", 9, "bold")).pack(side="left", padx=2, pady=3)

    # ─── Editor + Output (unchanged) ─────────────────────────────────
    ...
    def _bind_keys(self):
        for combo, fn in [
            ...
            ("<F7>", lambda e: self._run_external_lang('compile', 'C')),
            ("<F8>", lambda e: self._run_external_lang('run', 'C')),
            ("<F10>", lambda e: self._run_external_lang('build_run', 'C')),
            ("<F11>", lambda e: self._run_external_lang('assembly', 'C')),
            ("<Control-r>", lambda e: self._run_external('run')),
            ("<Control-e>", lambda e: self._run_external('compile')),
            ("<Control-b>", lambda e: self._run_external('build_run')),
        ]:
    ...
    def _update_title(self):
        name = os.path.basename(self.file_path) if self.file_path else "untitled" + LANG_CONFIG[self.current_lang.get()]['ext']
    ...
    def file_open(self):
        if self.modified:
            ...
            if r is None: return
            if r: self.file_save()
        p = filedialog.askopenfilename(filetypes=[("All files","*.*"), ("C files","*.c *.h *.cpp"), ("Python","*.py"), ("Java","*.java")])
        if not p: return
        try:
    ...
    def file_save_as(self):
        ext = LANG_CONFIG[self.current_lang.get()]['ext']
        p = filedialog.asksaveasfilename(defaultextension=ext,
            filetypes=[(f"{self.current_lang.get()} files", f"*{ext}"), ("All","*.*")])
        if not p: return
    ...
    # ─── Built‑in C Compile / Run ────────────────────────────────────
    def do_compile(self):
        self._clear_out()
        self._out("─── Compiling (built‑in C) ───\n", "info")
    ...
    def do_build_run(self):
        self._clear_out()
        self._out("─── Build & Run (built‑in C) ───\n", "info")
    ...
    def _about(self):
        messagebox.showinfo("About",
            ...
            "Built‑in: C compiler (no external tools)\n"
            "External: C, C++, Python, Java, JavaScript,\n"
            "          Ruby, Go, Rust, and more.\n\n"
            "Select language from toolbar or menu.\n"
            "Use Ctrl+R / Ctrl+E / Ctrl+B for external run/compile.\n\n"
            "Built‑in C shortcuts: F5=Compile, F6=Run, F9=Build+Run\n"
            "GCC shortcuts: F7..F11 for C/C++ (if GCC installed).\n\n"
            "CatGPT 2.x — Team Flames 2026")
