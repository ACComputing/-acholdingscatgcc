...
GCC EXTENSION (optional):
  - Use external GCC for real compilation (C/C++, assembly)
  - Compile with GCC (F7), Run with GCC (F8), Build+Run (F10)
  - Generate Assembly (F11) – saves .s file next to source
  - Cross‑platform (Windows/Linux/macOS) if GCC is installed
...
# ═══════════════════════════════════════════════════════════════════════
# INTERPRETER (Tree-Walking) – full original
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# GUI — Cat's GCC 0.1 (extended with GCC)
# ═══════════════════════════════════════════════════════════════════════
...
class CatsGCC_GUI:
    def __init__(self, root):
        ...
        # GCC extension: detect if gcc/g++ is available
    ...
    def _run_gcc_command(self, mode='build_run', lang='c'):
        source = self.editor.get("1.0", "end-1c")
        if not source.strip():
            self._out("No source code to compile.\n", "stderr")
            return
        ...
        if mode == 'assembly':
            cmd = [compiler, '-S', src_path, '-o', asm_path]

        self._out(f"$ {' '.join(cmd)}\n", "info")
        self._status("Compiling with GCC...", "info")

        try:
            proc = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
            if proc.returncode != 0:
                self._out(proc.stderr + '\n', "stderr")
                self._status("Compilation failed", "err")
                return

            if mode == 'assembly':
                self._out(f"Assembly generated: {asm_path}\n", "success")
                self._status("Assembly generated", "ok")
                return

            if mode == 'compile':
                self._out("Compilation successful.\n", "success")
                self._status("Compiled with GCC", "ok")
                return

            self._out("\n--- Running ---\n", "info")
            run_proc = subprocess.run([exe_path], capture_output=True, text=True, timeout=30)
            if run_proc.stdout:
                self._out(run_proc.stdout, "stdout")
            if run_proc.stderr:
                self._out(run_proc.stderr, "stderr")
            self._out(f"\nProgram exited with code {run_proc.returncode}\n", "success")
            self._status("Execution finished", "ok")

        except subprocess.TimeoutExpired:
            self._out("Error: Compilation/execution timed out.\n", "stderr")
            self._status("Timeout", "err")
        except FileNotFoundError:
            self._out(f"Error: {compiler} not found. Please install GCC.\n", "stderr")
            self._status("GCC not found", "err")
        except Exception as e:
            self._out(f"Unexpected error: {e}\n", "stderr")
    ...
    def _build_menu(self):
        ...
        gm = tk.Menu(mb, **mcfg)
        gm.add_command(label="Compile with GCC   F7",
                       command=lambda: self._run_gcc_command('compile', 'c'),
                       state='normal' if self.gcc_available else 'disabled')
        gm.add_command(label="Run with GCC       F8",
                       command=lambda: self._run_gcc_command('run', 'c'),
                       state='normal' if self.gcc_available else 'disabled')
        gm.add_command(label="Build & Run with GCC F10",
                       command=lambda: self._run_gcc_command('build_run', 'c'),
                       state='normal' if self.gcc_available else 'disabled')
        gm.add_separator()
        gm.add_command(label="Generate Assembly  F11",
                       command=lambda: self._run_gcc_command('assembly', 'c'),
                       state='normal' if self.gcc_available else 'disabled')
        gm.add_separator()
        gm.add_command(label="Compile C++ with GCC",
                       command=lambda: self._run_gcc_command('compile', 'cpp'),
                       state='normal' if self.gcc_available else 'disabled')
        gm.add_command(label="Run C++ with GCC",
                       command=lambda: self._run_gcc_command('run', 'cpp'),
                       state='normal' if self.gcc_available else 'disabled')
    ...
    def _build_toolbar(self):
        ...
        if self.gcc_available:
            tk.Frame(tb, bg=FG_GUTTER, width=1).pack(side="left", fill="y", padx=6, pady=4)
            tk.Button(tb, text="GCC RUN [F8]", command=lambda: self._run_gcc_command('run', 'c'),
    ...
    def _bind_keys(self):
        for combo, fn in [
            ...
            ("<F7>", lambda e: self._run_gcc_command('compile', 'c')),
            ("<F8>", lambda e: self._run_gcc_command('run', 'c')),
            ("<F10>", lambda e: self._run_gcc_command('build_run', 'c')),
            ("<F11>", lambda e: self._run_gcc_command('assembly', 'c')),
        ]:
    ...
    def _about(self):
        messagebox.showinfo("About",
            ...
            "GCC Extension: optional external GCC\n"
            "  for real compilation (C/C++, assembly).\n\n"
            "Shortcuts: F5=Compile F6=Run F9=Build+Run\n"
            "  F7=GCC Compile F8=GCC Run F10=GCC Build+Run\n"
            "  F11=GCC Assembly\n\n"
